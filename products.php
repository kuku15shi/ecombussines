...
if ($filter === 'top') $where .= " AND p.is_top = 1";
if ($filter === 'trending') $where .= " AND p.is_trending = 1";
...
<head>
  ...
  <title><?= $pageTitle ?> – LuxeStore</title>
  <meta name="description" content="Browse our premium product collection at LuxeStore.">
...
<div class="page-wrapper">
  <div class="container">
    ...
    <div class="breadcrumb">
      ...
      <?php if($category_slug): ?><a href="<?= SITE_URL ?>/products">Products</a><span class="separator"><i class="bi bi-chevron-right"></i></span><span class="current"><?= $pageTitle ?></span>
      <?php elseif($search): ?><a href="<?= SITE_URL ?>/products">Products</a><span class="separator"><i class="bi bi-chevron-right"></i></span><span class="current">Search Results</span>
    ...
    <div class="products-layout-wrapper">
      ...
      <aside class="filters-sidebar">
        <div class="glass-card" style="padding:1.5rem; position:sticky; top:90px;">
          <h3 style="font-size:1rem; font-weight:700; margin-bottom:1.25rem; display:flex; justify-content:space-between; align-items:center;">
            <span><i class="bi bi-sliders"></i> Filters</span>
            <div style="display:flex; gap:1rem; align-items:center;">
              <a href="<?= SITE_URL ?>/products" style="font-size:0.75rem; color:var(--text-muted); text-decoration:none;">Clear All</a>
              <button class="d-md-none" onclick="toggleFilters()" style="background:none; border:none; color:var(--text-primary); font-size:1.25rem;"><i class="bi bi-x"></i></button>
            </div>
          </h3>
          ...
          <form method="GET" action="<?= SITE_URL ?>/products" id="filterForm">
            ...
            <div style="margin-bottom:1.5rem;">
              <div style="font-size:0.8rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:1px; margin-bottom:0.875rem;">Categories</div>
              <div class="filter-group">
                <label class="filter-option <?= !$category_slug ? 'active' : '' ?>">
                  <input type="radio" name="category" value="" <?= !$category_slug ? 'checked' : '' ?> onchange="this.form.submit()"> All Categories
                </label>
                <?php foreach($categories as $cat): ?>
                <label class="filter-option <?= $category_slug === $cat['slug'] ? 'active' : '' ?>">
                  <input type="radio" name="category" value="<?= $cat['slug'] ?>" <?= $category_slug === $cat['slug'] ? 'checked' : '' ?> onchange="this.form.submit()">
            ...
            <!-- Price Range -->
            <div style="margin-bottom:1.5rem;">
              <div style="font-size:0.8rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:1px; margin-bottom:0.875rem;">Price Range</div>
              <div style="display:flex; gap:0.5rem; margin-bottom:0.75rem;">
                <input type="number" name="min_price" value="<?= isset($_GET['min_price']) ? htmlspecialchars($_GET['min_price']) : '' ?>" placeholder="Min" class="form-control" style="flex:1; padding:0.5rem 0.75rem; font-size:0.85rem;" step="0.01">
                <input type="number" name="max_price" value="<?= isset($_GET['max_price']) ? htmlspecialchars($_GET['max_price']) : '' ?>" placeholder="Max" class="form-control" style="flex:1; padding:0.5rem 0.75rem; font-size:0.85rem;" step="0.01">
              </div>
            </div>

            <!-- Filter Type -->
            <div style="margin-bottom:1.5rem;">
              <div style="font-size:0.8rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:1px; margin-bottom:0.875rem;">Product Type</div>
              <div class="filter-group">
                ...
                <label class="filter-option <?= $filter === $v ? 'active' : '' ?>">
                  <input type="radio" name="filter" value="<?= $v ?>" <?= $filter === $v ? 'checked' : '' ?> onchange="this.form.submit()"> <?= $l ?>
                </label>
                <?php endforeach; ?>
              </div>
            </div>

            <button type="submit" class="btn-primary-luxury" style="width:100%; justify-content:center; padding:0.65rem;">
              Apply Filters
            </button>
          </form>
        </div>
      ...
      <div style="flex:1; min-width:0;">
        <!-- Top Bar -->
        <div class="glass-card" style="padding:0.75rem 1.25rem; margin-bottom:1.5rem; display:flex; justify-content:space-between; align-items:center; gap:1rem; flex-wrap:wrap;">
          <div style="display:flex; align-items:center; gap:1rem;">
            <button class="btn-outline-luxury d-md-none" style="padding:0.5rem 1rem; font-size:0.875rem; border-radius:var(--radius-sm);" onclick="toggleFilters()">
        ...
        <div id="filterOverlay" onclick="toggleFilters()" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:1999; display:none; backdrop-filter:blur(4px);"></div>

        <!-- Products Grid -->
        <?php if(empty($products)): ?>
        <div class="glass-card" style="text-align:center; padding:5rem 2rem;">
          <div style="width:80px; height:80px; background:var(--glass); border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 1.5rem;">
          ...
          <p style="color:var(--text-muted); margin-bottom:2rem; font-size:0.95rem;">We couldn't find any products matching your current filters.</p>
          <a href="<?= SITE_URL ?>/products" class="btn-primary-luxury">
            <i class="bi bi-arrow-left"></i> View All Products
          </a>
        </div>
        <?php else: ?>
        <div class="products-grid grid-cols-3" id="productsGrid">
        ...
        <!-- Pagination -->
        <?php if($totalPages > 1): ?>
        <div style="display:flex; justify-content:center; gap:0.5rem; margin-top:2.5rem; flex-wrap:wrap;">
          <?php
          $qParams = $_GET;
          for($pi = 1; $pi <= $totalPages; $pi++):
            ...
            $isActive = $pi === $page;
          ?>
          <a href="<?= SITE_URL ?>/products?<?= http_build_query($qParams) ?>"
