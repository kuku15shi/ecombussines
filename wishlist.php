...
<head>
  ...
  <title>My Wishlist – MIZ MAX</title>
        ...
        <?php foreach ($items as $p):
          ...
          <div class="glass-card" style="padding:0; overflow:hidden; position:relative;" id="wl-<?= $p['id'] ?>">
            ...
            <button onclick="removeFromWishlist(<?= $p['id'] ?>)"
              style="position:absolute; top:0.75rem; right:0.75rem; background:rgba(255,101,132,0.2); border:1px solid rgba(255,101,132,0.4); border-radius:50%; width:34px; height:34px; color:var(--secondary); cursor:pointer; display:flex; align-items:center; justify-content:center;">
            ...
            <div style="padding:1rem;">
              ...
              <a href="<?= SITE_URL ?>/product/<?= $p['slug'] ?>" style="text-decoration:none; color:var(--text-primary);">
                <div style="font-weight:700; margin-bottom:0.5rem; font-size:0.9rem; line-height:1.4;">
                  <?= htmlspecialchars($p['name']) ?></div>
              </a>
              <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                  <div
                    style="font-weight:800; font-size:1.1rem; background:linear-gradient(135deg,var(--primary),var(--accent2)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;">
                ...
                <button onclick="addToCart(<?= $p['id'] ?>)" class="btn-primary-luxury"
                  style="padding:0.5rem 1rem; font-size:0.8rem;" <?= $p['stock'] <= 0 ? 'disabled' : '' ?>>
