...
<div class="page-wrapper">
  <div class="container">
    ...
    <h1 style="font-size:1.8rem; font-weight:800; margin-bottom:1.75rem;">
      <i class="bi bi-bag" style="background:linear-gradient(135deg,var(--primary),var(--accent2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;"></i>
      Shopping Cart <span style="font-size:1rem; color:var(--text-muted); font-weight:500;">(<?= count($cartItems) ?> items)</span>
    </h1>
    ...
    <div class="glass-card" style="text-align:center; padding:5rem 2rem;">
      ...
      <p style="color:var(--text-muted); margin-bottom:2rem;">Discover our premium collection and add items to your cart</p>
    ...
    <div class="cart-layout-grid">
      ...
      <div style="display:flex; flex-direction:column; gap:1rem;">
        ...
        <div class="glass-card cart-item" id="cart-item-<?= $item['cartKey'] ?>">
          <a href="<?= SITE_URL ?>/product/<?= $p['slug'] ?>">
            <img src="<?= UPLOAD_URL . $firstImg ?>" onerror="this.src='<?= SITE_URL ?>/assets/img/default_product.jpg'" class="cart-item-img" alt="">
          </a>
          <div class="cart-item-info">
            ...
            <div style="display:flex; gap:0.6rem; margin:0.4rem 0;">
              <?php if($item['size']): ?>
              <span style="font-size:0.75rem; background:var(--glass); border:1px solid var(--glass-border); padding:2px 8px; border-radius:4px; font-weight:600;">Size: <?= strtoupper($item['size']) ?></span>
              <?php endif; ?>
              <?php if($item['color']): ?>
              <span style="font-size:0.75rem; background:var(--glass); border:1px solid var(--glass-border); padding:2px 8px; border-radius:4px; font-weight:600; display:flex; align-items:center; gap:4px;">
                Color: <span style="width:10px; height:10px; border-radius:50%; background:<?= $item['color'] ?>;"></span>
              </span>
              <?php endif; ?>
            </div>
            <?php endif; ?>
      ...
      <div class="glass-card cart-summary">
        ...
        <div style="display:flex; flex-direction:column; gap:0.75rem; margin-bottom:1.5rem;">
          ...
          <div style="display:flex; justify-content:space-between; color:var(--text-secondary); font-size:0.9rem;">
            <span>Shipping</span>
            <span id="summaryShipping"><?= $shipping === 0 ? '<span style="color:var(--success)">FREE</span>' : formatPrice($shipping) ?></span>
          </div>
          <div style="display:flex; justify-content:space-between; color:var(--text-secondary); font-size:0.9rem;">
            <span>GST (<?= GST_PERCENT ?>%)</span>
            <span id="summaryGst"><?= formatPrice($gst) ?></span>
          </div>
          <div style="border-top:1px solid var(--border); padding-top:0.875rem; display:flex; justify-content:space-between; font-weight:800; font-size:1.2rem;">
            <span>Total</span>
            <span style="background:linear-gradient(135deg,var(--primary),var(--accent2)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;" id="summaryTotal"><?= formatPrice($total) ?></span>
          </div>
        </div>
        <?php if($subtotal >= FREE_SHIPPING_ABOVE || $subtotal === 0): ?>
        <div style="background:rgba(67,233,123,0.1); border:1px solid rgba(67,233,123,0.3); border-radius:var(--radius-sm); padding:0.6rem 1rem; margin-bottom:1rem; font-size:0.8rem; color:var(--success);">
          <i class="bi bi-truck"></i> You qualify for free shipping!
        </div>
        <?php else: ?>
        <div style="background:rgba(247,183,49,0.1); border:1px solid rgba(247,183,49,0.2); border-radius:var(--radius-sm); padding:0.6rem 1rem; margin-bottom:1rem; font-size:0.8rem; color:var(--warning);">
          <i class="bi bi-truck"></i> Add <?= formatPrice(FREE_SHIPPING_ABOVE - $subtotal) ?> more for free shipping
        </div>
        <?php endif; ?>
        <a href="<?= SITE_URL ?>/checkout" class="btn-primary-luxury" style="width:100%; justify-content:center; font-size:1rem; padding:0.875rem;">
          <i class="bi bi-lock-fill"></i> Proceed to Checkout
        </a>
        <a href="<?= SITE_URL ?>/products" class="btn-outline-luxury" style="width:100%; justify-content:center; margin-top:0.75rem;">
