...
<div class="admin-layout">
  ...
  <div class="main-content">
    ...
    <div class="content-area">
      ...
      <!-- Status Tabs -->
      <div style="display:flex; gap:0.5rem; flex-wrap:wrap; margin-bottom:1.5rem; background:var(--glass); border:1px solid var(--glass-border); border-radius:var(--radius); padding:0.5rem;">
        <a href="orders.php" class="btn-primary btn-sm <?= !$statusFilter?'':'default' ?>" style="<?= !$statusFilter?'':'background:transparent;color:var(--text-muted);box-shadow:none;' ?>">All (<?= $total ?>)</a>
        <?php foreach($statuses as $s):
          $cntStmt = $pdo->prepare("SELECT COUNT(*) as c FROM orders WHERE order_status=?");
          $cntStmt->execute([$s]);
          $cnt = $cntStmt->fetch()['c'];
        ?>
        <a href="orders.php?status=<?= $s ?>" class="btn-primary btn-sm" style="<?= $statusFilter===$s?'':'background:var(--glass);color:var(--text-muted);border:1px solid var(--glass-border);box-shadow:none;' ?>">
          <?= ucfirst($s) ?> (<?= $cnt ?>)
        </a>
        <?php endforeach; ?>
      </div>
