...
// 1. You may also like (Same category, tags, or similar price range)
$price_min = $p['price'] * 0.7;
$price_max = $p['price'] * 1.3;
$stmtYouMayLike = $pdo->prepare("SELECT p.*, c.name as cat_name, c.slug as cat_slug FROM products p LEFT JOIN categories c ON p.category_id=c.id WHERE p.is_active=1 AND p.id != ? AND (p.category_id = ? OR p.price BETWEEN ? AND ?) ORDER BY (p.tags LIKE ?) DESC, RAND() LIMIT 4");
$stmtYouMayLike->execute([$p['id'], $p['category_id'], $price_min, $price_max, isset($p['tags']) ? '%' . $p['tags'] . '%' : '']);
$you_may_like = $stmtYouMayLike->fetchAll();

// 2. Trending Products
$trending_products = getTrendingProducts($pdo, 4);

// 3. Frequently Bought Together
$stmtFreq = $pdo->prepare("SELECT p.*, c.name as cat_name, c.slug as cat_slug FROM products p LEFT JOIN categories c ON p.category_id=c.id WHERE p.is_active=1 AND p.id != ? AND p.category_id = ? ORDER BY p.sales_count DESC, p.review_count DESC LIMIT 3");
$stmtFreq->execute([$p['id'], $p['category_id']]);
$freq_bought = $stmtFreq->fetchAll();
...
// Fetch Related Shoppable Reels
$suggestedReels = [];
try {
  $stmt = $pdo->prepare("SELECT v.*, p.name as product_name, p.slug as product_slug 
                         FROM videos v 
                         LEFT JOIN products p ON v.product_id = p.id 
                         WHERE v.is_active = 1 AND v.product_id = ? 
                         ORDER BY v.created_at DESC LIMIT 8");
  $stmt->execute([$p['id']]);
  $suggestedReels = $stmt->fetchAll();

  // Fall back to general active reels if none specific to this product
  if (empty($suggestedReels)) {
...
foreach ($variants as $v) {
  if ($v['color'])
    $variantMap[strtolower(trim($v['color']))] = $v['image'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
  validateCsrf();
  if (!$currentUser) {
    header('Location: ' . SITE_URL . '/login?redirect=product/' . urlencode($slug));
    exit;
  }
...
<head>
  ...
  <title><?= htmlspecialchars($p['name']) ?> – MIZ MAX</title>
  <meta name="description"
    content="<?= htmlspecialchars($p['short_description'] ?? substr(strip_tags($p['description']), 0, 160)) ?>">
  ...
  <style>
    ...
    .gallery-main .video-container.active {
      display: block;
    }

    .gallery-main:hover img.active {
  ...
  <div class="container">
      ...
      <div class="product-detail-grid">
        ...
        <div class="img-gallery">
            ...
            <div class="gallery-thumbnails">
              <?php foreach ($imgs as $i => $img): ?>
                <div class="gallery-thumb <?= $i === 0 ? 'active' : '' ?>"
                  onclick="setMainContent('image', '<?= UPLOAD_URL . $img ?>', this)">
                  <img src="<?= UPLOAD_URL . $img ?>" onerror="this.src='<?= SITE_URL ?>/assets/img/default_product.jpg'"
                    alt="">
                </div>
              <?php endforeach; ?>
          ...
          <div class="gallery-main">
            <img src="<?= UPLOAD_URL . $imgs[0] ?>" id="mainImg" class="active"
            ...
            <?php if ($videoEmbed): ?>
              <div id="mainVideo" class="video-container">
                <?php if ($videoEmbed['type'] === 'youtube' || $videoEmbed['type'] === 'iframe'): ?>
                  <iframe src="<?= htmlspecialchars($videoEmbed['embed']) ?>" style="width:100%; height:100%; border:0;"
        ...
        <div>
          <div
            style="font-size:0.78rem; color:var(--primary); font-weight:700; text-transform:uppercase; letter-spacing:1.5px; margin-bottom:0.5rem;">
            <?= htmlspecialchars($p['cat_name']) ?></div>
          <h1 style="font-size:1.8rem; font-weight:800; line-height:1.2; margin-bottom:0.75rem;">
          ...
          <div style="display:flex; align-items:center; gap:0.75rem; margin-bottom:1.25rem;">
            <div class="stars">
              <?php for ($i = 1; $i <= 5; $i++): ?>
                <i class="bi <?= $i <= $fullStars ? 'bi-star-fill' : ($halfStar && $i == $fullStars + 1 ? 'bi-star-half' : 'bi-star') ?>"
                  style="color:var(--gold);"></i>
              <?php endfor; ?>
            </div>
            <span style="font-size:0.85rem; color:var(--text-muted);"><?= $rating ?> (<?= count($reviews) ?>
              reviews)</span>
            <?php if ($p['stock'] > 0): ?>
              <span
                style="font-size:0.8rem; color:var(--success); background:rgba(67,233,123,0.1); padding:0.2rem 0.7rem; border-radius:50px; border:1px solid rgba(67,233,123,0.3);">✓
                In Stock (<?= $p['stock'] ?>)</span>
            <?php else: ?>
              <span
                style="font-size:0.8rem; color:var(--danger); background:rgba(255,101,132,0.1); padding:0.2rem 0.7rem; border-radius:50px; border:1px solid rgba(255,101,132,0.3);">✗
                Out of Stock</span>
            <?php endif; ?>
          </div>
          ...
          <div style="display:flex; align-items:center; gap:1rem; margin-bottom:1.75rem;">
            <span
              style="font-size:2.2rem; font-weight:900; background:linear-gradient(135deg,var(--primary),var(--accent2)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;"><?= formatPrice($discountedPrice) ?></span>
            <?php if ($p['discount_percent'] > 0): ?>
              <span
                style="font-size:1.1rem; color:var(--text-muted); text-decoration:line-through;"><?= formatPrice($p['price']) ?></span>
              <span
                style="background:linear-gradient(135deg,var(--secondary),var(--accent2)); color:#fff; font-size:0.8rem; font-weight:700; padding:0.3rem 0.8rem; border-radius:50px;"><?= intval($p['discount_percent']) ?>%
                OFF</span>
            <?php endif; ?>
          </div>
          ...
          <?php if ($p['short_description']): ?>
            <p style="color:var(--text-secondary); line-height:1.7; margin-bottom:1.5rem;">
              <?= htmlspecialchars($p['short_description']) ?></p>
          <?php endif; ?>
          ...
          if (isAffiliateLoggedIn()):
            ...
            if ($affData):
              ...
              <div class="glass-card mb-4"
                style="border: 2px dashed var(--primary); background: rgba(108, 99, 255, 0.05); padding: 1.25rem;">
                <div class="d-flex justify-content-between align-items-center mb-2">
                  <span style="font-size: 0.75rem; font-weight: 800; color: var(--primary); letter-spacing: 1px;"><i
                      class="bi bi-gem me-1"></i> AFFILIATE TOOLS</span>
                  <span class="badge bg-success" style="font-size: 0.65rem;">Earn
                    <?= formatPrice($discountedPrice * ($itemComm / 100)) ?></span>
                </div>
                <div class="input-group mb-2">
                  <input type="text" class="form-control form-control-sm border-0 bg-white" id="affLinkInput"
                    value="<?= $affLink ?>" readonly style="font-size: 0.8rem; border-radius: 8px 0 0 8px;">
                  <button class="btn btn-primary btn-sm" onclick="copyAffLink()"
                    style="border-radius: 0 8px 8px 0;">Copy</button>
                </div>
                <div class="d-flex gap-2">
                  <a href="https://wa.me/?text=Check this out: <?= urlencode($affLink) ?>" target="_blank"
                    class="btn btn-outline-success btn-sm flex-grow-1" style="font-size: 0.75rem; border-radius: 6px;"><i
                      class="bi bi-whatsapp"></i> WhatsApp</a>
                  <a href="https://twitter.com/intent/tweet?url=<?= urlencode($affLink) ?>" target="_blank"
                    class="btn btn-outline-info btn-sm flex-grow-1" style="font-size: 0.75rem; border-radius: 6px;"><i
                      class="bi bi-twitter-x"></i> Tweet</a>
                </div>
              </div>
              <script>
          ...
          <?php if (!empty($variants)): ?>
              ...
              <div style="display:flex; gap:0.75rem; flex-wrap:wrap;">
                <?php foreach ($variants as $v): ?>
                  <div class="color-thumb-wrapper"
                    onclick="selectVariant('color', '<?= htmlspecialchars($v['color']) ?>', document.querySelector('.color-option[title=\'<?= htmlspecialchars($v['color']) ?>\']'))"
                    style="cursor:pointer; width:60px; height:80px; border-radius:8px; overflow:hidden; border:2px solid transparent; transition:var(--transition); background:var(--glass);">
            ...
            <style>
              .color-thumb-wrapper:hover {
                border-color: var(--primary);
                transform: translateY(-2px);
              }

              .color-thumb-wrapper.active {
                border-color: var(--primary);
          ...
          <?php if ($p['stock'] > 0): ?>
            <div style="display:flex; align-items:center; gap:0.85rem; margin-bottom:1.25rem; flex-wrap:wrap;">
              ...
              <button class="btn-outline-luxury" onclick="addToCartWithQty(<?= $p['id'] ?>)"
                style="flex:1; justify-content:center; border-radius:var(--radius-sm); font-size: 0.85rem; padding: 0.75rem 1rem;">
                <i class="bi bi-bag-plus"></i> Add to Cart
              </button>
              <button class="btn-buy-now" onclick="buyNow(<?= $p['id'] ?>)"
                style="flex:1.2; justify-content:center; padding: 0.75rem 1rem;">
                <i class="bi bi-lightning-fill"></i> Buy Now
              </button>
            </div>
            <div style="display:flex; gap:0.75rem; margin-bottom:1.5rem;">
              <button class="btn-outline-luxury <?= $inWishlist ? 'active' : '' ?>" id="wishlistBtn"
                onclick="toggleWishlistDetail(<?= $p['id'] ?>)" style="flex:1; justify-content:center;">
          ...
          <div style="margin-top:2rem; padding-top:1.5rem; border-top:1px solid var(--border);">
            <div
              style="font-size:0.75rem; font-weight:800; color:var(--text-muted); text-transform:uppercase; letter-spacing:1px; margin-bottom:1rem; display:flex; align-items:center; gap:0.5rem;">
              <i class="bi bi-shield-lock" style="color:var(--primary);"></i> SAFETY & SECURITY
            </div>
            <div class="grid-2" style="gap:1rem;">
              <div
                style="display:flex; align-items:center; gap:0.6rem; background:rgba(108,99,255,0.05); border:1px solid rgba(108,99,255,0.1); border-radius:12px; padding:0.75rem 1rem;">
              ...
              <div
                style="display:flex; align-items:center; gap:0.6rem; background:rgba(67,233,123,0.05); border:1px solid rgba(67,233,123,0.1); border-radius:12px; padding:0.75rem 1rem;">
              ...
              <div
                style="display:flex; align-items:center; gap:0.6rem; background:rgba(255,193,7,0.05); border:1px solid rgba(255,193,7,0.1); border-radius:12px; padding:0.75rem 1rem;">
    ...
    <div class="glass-card" style="padding:0; margin-bottom:2rem; overflow:hidden;">
      <div style="display:flex; border-bottom:1px solid var(--border); padding:0 1.5rem; flex-wrap:wrap;">
        ...
        <?php if ($videoEmbed): ?>
          <button class="tab-btn" id="videoTabBtn" onclick="switchTab('video',this)"><i class="bi bi-play-circle-fill"
              style="color:var(--primary); margin-right:0.3rem;"></i>Video</button>
        <?php endif; ?>
        <?php if (!empty($p['model_3d'])): ?>
          <button class="tab-btn" id="modelTabBtn" onclick="switchTab('3d',this); init3DViewer();"><i class="bi bi-box"
              style="color:var(--primary); margin-right:0.3rem;"></i>3D View</button>
        <?php endif; ?>
      ...
      <div style="padding:2rem;">
        ...
        <?php if ($videoEmbed): ?>
          <div id="tab-video" class="tab-content">
            <div
              style="font-size:0.9rem; color:var(--text-muted); margin-bottom:1.25rem; display:flex; align-items:center; gap:0.5rem;">
              <i class="bi bi-camera-video" style="color:var(--primary);"></i>
              Product Video
            </div>
            <?php if ($videoEmbed['type'] === 'youtube' || $videoEmbed['type'] === 'iframe'): ?>
              <div
                style="position:relative; width:100%; padding-bottom:56.25%; border-radius:var(--radius); overflow:hidden; background:#000;">
                <iframe src="<?= htmlspecialchars($videoEmbed['embed']) ?>"
                  style="position:absolute; top:0; left:0; width:100%; height:100%; border:0;"
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowfullscreen loading="lazy" title="<?= htmlspecialchars($p['name']) ?> video">
                </iframe>
              </div>
            <?php elseif ($videoEmbed['type'] === 'file'): ?>
              <div style="border-radius:var(--radius); overflow:hidden; background:#000;">
                <video controls preload="metadata" style="width:100%; display:block; max-height:500px;"
                  poster="<?= UPLOAD_URL . ($imgs[0] ?? '') ?>">
                  <source src="<?= htmlspecialchars($videoEmbed['url']) ?>"
                    type="video/<?= $videoEmbed['ext'] === 'mp4' ? 'mp4' : ($videoEmbed['ext'] === 'webm' ? 'webm' : 'ogg') ?>">
        ...
        <?php if (!empty($p['model_3d'])): ?>
          <div id="tab-3d" class="tab-content">
            ...
            <div id="canvas-container"
              style="width:100%; height:500px; background:#f0f0f0; border-radius:var(--radius); overflow:hidden; position:relative; cursor:grab;">
              <div id="loader-3d"
                style="position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(255,255,255,0.8); z-index:10;">
                <div class="spinner-border text-primary" role="status"></div>
                <span class="ms-2">Loading 3D model...</span>
              </div>
            </div>
            <div style="margin-top:1rem; display:flex; gap:1rem; justify-content:center;">
              <span style="font-size:0.75rem; color:var(--text-muted);"><i class="bi bi-mouse me-1"></i> Left click:
                Rotate</span>
              <span style="font-size:0.75rem; color:var(--text-muted);"><i class="bi bi-mouse2 me-1"></i> Right click:
                Pan</span>
              <span style="font-size:0.75rem; color:var(--text-muted);"><i class="bi bi-arrows-expand me-1"></i> Scroll:
                Zoom</span>
            </div>
          </div>
        ...
        <div id="tab-reviews" class="tab-content">
          ...
          <div style="display:flex; gap:2rem; align-items:center; margin-bottom:2rem; flex-wrap:wrap;">
            <div style="text-align:center;">
              <div
                style="font-size:4rem; font-weight:900; background:linear-gradient(135deg,var(--gold),#F97316); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; line-height:1;">
              ...
              <div style="font-size:0.8rem; color:var(--text-muted); margin-top:0.25rem;"><?= count($reviews) ?> reviews
              </div>
            </div>
            <!-- Rating Bars -->
            <div style="flex:1;">
              <?php for ($i = 5; $i >= 1; $i--):
                ...
                <div style="display:flex; align-items:center; gap:0.75rem; margin-bottom:0.4rem;">
                  ...
                  <div style="flex:1; height:6px; background:var(--glass); border-radius:3px; overflow:hidden;">
                    <div
                      style="width:<?= $pct ?>%; height:100%; background:linear-gradient(135deg,var(--gold),#F97316); border-radius:3px;">
                    </div>
                  </div>
          ...
          <?php if ($currentUser): ?>
            <details style="margin-bottom:1.5rem;">
              <summary
                style="cursor:pointer; font-weight:700; color:var(--primary); list-style:none; display:flex; align-items:center; gap:0.5rem; margin-bottom:1rem;">
                <i class="bi bi-pencil-square"></i> Write a Review
              </summary>
              <form method="POST"
                style="background:var(--glass); border:1px solid var(--glass-border); border-radius:var(--radius); padding:1.5rem; margin-top:0.75rem;">
                <?= csrfField() ?>
                ...
                <div class="review-card">
                  <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:0.75rem;">
                    <div style="display:flex; align-items:center; gap:0.75rem;">
                      <div
                        style="width:40px; height:40px; border-radius:50%; background:linear-gradient(135deg,var(--primary),var(--accent2)); display:flex; align-items:center; justify-content:center; font-weight:700; font-size:1rem;">
                        <?= strtoupper(substr($rev['user_name'] ?? 'U', 0, 1)) ?>
                      </div>
                      <div>
                        <div style="font-weight:600; font-size:0.875rem;">
                          <?= htmlspecialchars($rev['user_name'] ?? 'Anonymous') ?></div>
                        <div class="stars" style="margin-top:2px;">
                          <?php for ($i = 1; $i <= 5; $i++): ?>
                            <i class="bi bi-star<?= $i <= $rev['rating'] ? '-fill' : '' ?>"
                              style="color:var(--gold); font-size:0.72rem;"></i>
                          <?php endfor; ?>
                        </div>
                      </div>
                    </div>
      ...
      <section style="margin-bottom:3rem;">
        <div class="section-header-flex">
          <div>
            ...
            <h2 class="section-title" style="font-size:1.5rem;">Related Shoppable Reels</h2>
          </div>
          <a href="<?= SITE_URL ?>/videos.php" class="btn-outline-luxury">Watch All <i class="bi bi-play-circle"></i></a>
        </div>
        <style>
          .reels-scroller {
            display: flex;
        ...
        <div class="reels-scroller">
          <?php foreach ($suggestedReels as $reel): ?>
            <a href="<?= SITE_URL ?>/videos.php" class="reel-card">
              ...
              <div class="reel-content">
                ...
                <?php if ($reel['product_name']): ?>
                  <div style="font-size:0.75rem; color:#4ade80; font-weight:800; margin-top:6px;"><i
                      class="bi bi-cart-check"></i> Featured</div>
                <?php endif; ?>
              </div>
            </a>
          <?php endforeach; ?>
    ...
    <?php if (!empty($freq_bought)): ?>
      <section
        style="margin-bottom:3rem; background:var(--glass); border:1px solid var(--glass-border); padding:2rem; border-radius:var(--radius);">
        <div class="section-header" style="text-align:left; margin-bottom:1.5rem;">
          <h2 class="section-title" style="font-size:1.5rem;"><i class="bi bi-cart-plus"
              style="color:var(--primary);"></i> Frequently Bought Together</h2>
        </div>
        <div style="display:flex; flex-wrap:wrap; gap:1.5rem; align-items:center;">
          <div style="flex:1; min-width:200px; display:flex; gap:1rem; align-items:center;">
            <!-- Main Product -->
            <div style="position:relative; width:120px;">
              <img src="<?= UPLOAD_URL . $imgs[0] ?>"
                style="border-radius:12px; width:100%; object-fit:cover; border:2px solid var(--primary);">
              <div
                style="position:absolute; top:-10px; right:-10px; background:var(--primary); color:#fff; width:24px; height:24px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.8rem;">
                <i class="bi bi-check2"></i></div>
            </div>
            <i class="bi bi-plus-lg" style="font-size:1.5rem; color:var(--text-muted);"></i>
            <!-- FBT Products -->
            <?php foreach ($freq_bought as $i => $fb):
              $fb_imgs = getProductImages($fb['images']); ?>
              <a href="<?= SITE_URL ?>/product/<?= $fb['slug'] ?>" style="position:relative; width:100px; display:block;">
                <img src="<?= UPLOAD_URL . $fb_imgs[0] ?>"
                  style="border-radius:12px; width:100%; object-fit:cover; border:1px solid var(--border);">
              </a>
              <?php if ($i < count($freq_bought) - 1): ?> <i class="bi bi-plus-lg"
                  style="font-size:1.5rem; color:var(--text-muted);"></i> <?php endif; ?>
            <?php endforeach; ?>
          </div>
          <div>
            <!-- Total calculation could be done here -->
            <button class="btn-primary-luxury" onclick="showToast('Bundle added to cart!', 'success')"
              style="padding:0.75rem 1.5rem;">Add Bundle to Cart</button>
          </div>
        </div>
      </section>
    <?php endif; ?>
    ...
    <?php if (!empty($trending_products)): ?>
      <section style="margin-bottom:3rem;">
        <div class="section-header" style="text-align:left; margin-bottom:1.5rem;">
          <h2 class="section-title" style="font-size:1.5rem;">Trending Now</h2>
        </div>
        <div class="products-grid">
...
<script>
  ...
  function buyNow(productId) {
    ...
    fetch(_siteUrl + '/ajax/cart.php', {
      ...
      if (data.success) {
        window.location.href = _siteUrl + '/checkout';
      } else {
